...
class IntraReader : public cyber::Reader<MessageT> {
 public:
  ...
  IntraReader(const cyber::proto::RoleAttributes& attr, const Callback& callback);
  virtual ~IntraReader();
...
template <typename MessageT>
IntraReader<MessageT>::IntraReader(const cyber::proto::RoleAttributes& attr,
...
void IntraReader<MessageT>::OnMessage(const MessagePtr& msg_ptr) {
  ...
  this->latest_recv_time_sec_ = cyber::Time::Now().ToSecond();
