...
function apollo_environ_setup() {
    ...
    if [ ! -f "${APOLLO_PROFILE}" ]; then
        cp -f /opt/cyber/rcfiles/apollo.sh.sample "${APOLLO_PROFILE}"
...
function download_if_not_cached {
    ...
    if [ -f "${pkg_name}" ];then
        info "Local file cache hit ${pkg_name}..."
        if _checksum_check_pass "${pkg_name}" "${expected_cs}"; then
            ok "will use local file."
            return
        else
            warning "Found ${pkg_name} in local file cache, but checksum mismatch."
            rm -f "${pkg_name}"
        fi
    fi
