...
void MessageCallback(
    const std::shared_ptr<cyber::examples::proto::Chatter>& msg) {
...
int main(int argc, char* argv[]) {
  ...
  // create listener
  auto listener =
      listener_node->CreateReader<cyber::examples::proto::Chatter>(
...
- `ros::init(argc, argv, "listener");` --> `cyber::Init(argv[0]);`
- `ros::NodeHandle n;` --> `auto listener_node = cyber::CreateNode("listener");`
...
- `ros::spin();` --> `cyber::WaitForShutdown();`
...
using cyber::examples::proto::Chatter;

int main(int argc, char *argv[]) {
  // init cyber framework
  cyber::Init(argv[0]);
  // create talker node
  auto talker_node = cyber::CreateNode("talker");
  // create talker
