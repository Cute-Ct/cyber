...
#include "python/internal/py_cyber.h"
...
using cyber::Node;
using cyber::PyChannelUtils;
using cyber::PyClient;
using cyber::PyNode;
using cyber::PyReader;
using cyber::PyService;
using cyber::PyServiceUtils;
using cyber::PyWriter;
...
static PyObject *cyber_py_init(PyObject *self, PyObject *args) {
  ...
  bool is_init = cyber::py_init(module_name);
  if (is_init) {
    Py_RETURN_TRUE;
  } else {
    Py_RETURN_FALSE;
...
static PyObject *cyber_py_is_shutdown(PyObject *self, PyObject *args) {
  bool is_shutdown = cyber::py_is_shutdown();
  if (is_shutdown) {
    Py_RETURN_TRUE;
  } else {
    Py_RETURN_FALSE;
...
PyObject *cyber_PyNodeUtils_get_active_nodes(PyObject *self, PyObject *args) {
  ...
  std::vector<std::string> nodes_name =
      cyber::PyNodeUtils::get_active_nodes(sleep_s);
...
PyObject *cyber_PyNodeUtils_get_node_attr(PyObject *self, PyObject *args) {
  ...
  const std::string node_attr =
      cyber::PyNodeUtils::get_node_attr(name, sleep_s);
...
PyObject *cyber_PyNodeUtils_get_readersofnode(PyObject *self, PyObject *args) {
  ...
  std::vector<std::string> readers_channel =
      cyber::PyNodeUtils::get_readersofnode(name, sleep_s);
...
PyObject *cyber_PyNodeUtils_get_writersofnode(PyObject *self, PyObject *args) {
  ...
  std::vector<std::string> writers_channel =
      cyber::PyNodeUtils::get_writersofnode(name, sleep_s);
