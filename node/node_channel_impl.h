...
class NodeChannelImpl {
 ...
 private:
  ...
  template <typename MessageT>
  void FillInAttr(cyber::proto::RoleAttributes* attr);

  bool is_reality_mode_;
  std::string node_name_;
  cyber::proto::RoleAttributes node_attr_;
  NodeManagerPtr node_manager_ = nullptr;
};

template <typename MessageT>
auto NodeChannelImpl::CreateWriter(const cyber::proto::RoleAttributes& role_attr)
    -> std::shared_ptr<Writer<MessageT>> {
  if (!role_attr.has_channel_name() || role_attr.channel_name().empty()) {
    AERROR << "Can't create a writer with empty channel name!";
    return nullptr;
  }
  cyber::proto::RoleAttributes new_attr(role_attr);
...
auto NodeChannelImpl::CreateWriter(const std::string& channel_name)
    -> std::shared_ptr<Writer<MessageT>> {
  cyber::proto::RoleAttributes role_attr;
...
template <typename MessageT>
auto NodeChannelImpl::CreateReader(const cyber::proto::RoleAttributes& role_attr,
                                   const CallbackFunc<MessageT>& reader_func,
                                   uint32_t pending_queue_size)
    -> std::shared_ptr<Reader<MessageT>> {
  if (!role_attr.has_channel_name() || role_attr.channel_name().empty()) {
...
template <typename MessageT>
void NodeChannelImpl::FillInAttr(cyber::proto::RoleAttributes* attr) {
