...
class RoleBase {
 public:
  RoleBase();
  explicit RoleBase(const cyber::proto::RoleAttributes& attr,
                    uint64_t timestamp_ns = 0);
  virtual ~RoleBase() = default;

  virtual bool Match(const cyber::proto::RoleAttributes& target_attr) const;
  bool IsEarlierThan(const RoleBase& other) const;

  const cyber::proto::RoleAttributes& attributes() const { return attributes_; }
  void set_attributes(const cyber::proto::RoleAttributes& attr) { attributes_ = attr; }

  uint64_t timestamp_ns() const { return timestamp_ns_; }
  void set_timestamp_ns(uint64_t timestamp_ns) { timestamp_ns_ = timestamp_ns; }

 protected:
  cyber::proto::RoleAttributes attributes_;
...
class RoleWriter : public RoleBase {
 public:
  RoleWriter() {}
  explicit RoleWriter(const cyber::proto::RoleAttributes& attr,
                      uint64_t timestamp_ns = 0);
  virtual ~RoleWriter() = default;

  bool Match(const cyber::proto::RoleAttributes& target_attr) const override;
};

class RoleServer : public RoleBase {
 public:
  RoleServer() {}
  explicit RoleServer(const cyber::proto::RoleAttributes& attr,
                      uint64_t timestamp_ns = 0);
