...
#include "service_discovery/container/warehouse_base.h"
...
class SingleValueWarehouse : public WarehouseBase {
 public:
  ...
  void Remove(const cyber::proto::RoleAttributes& target_attr) override;

  bool Search(uint64_t key) override;
  bool Search(uint64_t key, RolePtr* first_matched_role) override;
  bool Search(uint64_t key,
              cyber::proto::RoleAttributes* first_matched_role_attr) override;
  bool Search(uint64_t key, std::vector<RolePtr>* matched_roles) override;
  bool Search(uint64_t key,
              std::vector<cyber::proto::RoleAttributes>* matched_roles_attr) override;

  bool Search(const cyber::proto::RoleAttributes& target_attr) override;
  bool Search(const cyber::proto::RoleAttributes& target_attr,
  ...
  void GetAllRoles(std::vector<cyber::proto::RoleAttributes>* roles_attr) override;
