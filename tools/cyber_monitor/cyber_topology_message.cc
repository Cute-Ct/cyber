...
#include "tools/cyber_monitor/cyber_topology_message.h"
...
#include "tools/cyber_monitor/general_channel_message.h"
#include "tools/cyber_monitor/screen.h"
...
void CyberTopologyMessage::TopologyChanged(
    const cyber::proto::ChangeMsg& changeMsg) {
  if (::cyber::proto::OperateType::OPT_JOIN ==
      changeMsg.operate_type()) {
    bool isWriter = true;
    if (::cyber::proto::RoleType::ROLE_READER == changeMsg.role_type())
      isWriter = false;
    ...
    if (iter != all_channels_map_.cend() &&
      ...
      if (::cyber::proto::RoleType::ROLE_WRITER ==
          changeMsg.role_type()) {
...
void CyberTopologyMessage::AddReaderWriter(
    const cyber::proto::RoleAttributes& role, bool isWriter) {
  ...
  if (!GeneralChannelMessage::IsErrorCode(channel_msg)) {
    if (isWriter) {
      if (msgTypeName != cyber::message::MessageType<
                             cyber::message::RawMessage>()) {
        channel_msg->set_message_type(msgTypeName);
      }
