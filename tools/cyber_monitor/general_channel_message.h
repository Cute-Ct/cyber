...
#include "tools/cyber_monitor/general_message_base.h"
 ...
 private:
  explicit GeneralChannelMessage(const std::string& node_name,
      ...
      : GeneralMessageBase(parent),
        ...
        message_type_(),
        frame_counter_(0),
        last_time_(cyber::Time::MonoTime()),
  ...
  std::shared_ptr<cyber::message::RawMessage> CopyMsgPtr(void) const {
  ...
  cyber::Time last_time_;
  cyber::Time msg_time_;
  cyber::Time time_last_calc_ = cyber::Time::MonoTime();

  std::unique_ptr<cyber::Node> channel_node_;
  ...
  std::shared_ptr<cyber::message::RawMessage> channel_message_;
  std::shared_ptr<cyber::Reader<cyber::message::RawMessage>>
      channel_reader_;
