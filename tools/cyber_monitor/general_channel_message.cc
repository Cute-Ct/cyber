...
#include "tools/cyber_monitor/general_message.h"
#include "tools/cyber_monitor/screen.h"

namespace {
constexpr int ReaderWriterOffset = 4;
using cyber::record::kGB;
using cyber::record::kKB;
using cyber::record::kMB;
}  // namespace
...
double GeneralChannelMessage::frame_ratio(void) {
  ...
  auto time_now = cyber::Time::MonoTime();
...
GeneralChannelMessage* GeneralChannelMessage::OpenChannel(
  ...
  channel_node_ = cyber::CreateNode(node_name_);
  ...
  channel_reader_ =
      channel_node_->CreateReader<cyber::message::RawMessage>(
...
void GeneralChannelMessage::RenderDebugString(const Screen* s, int key,
  ...
  if (has_message_come()) {
    if (raw_msg_class_ == nullptr) {
      auto rawFactory = cyber::message::ProtobufFactory::Instance();
